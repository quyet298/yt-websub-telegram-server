...
router.get("/", (req, res) => {
  const html = [
    ...
    '        acc.feeds.forEach(feed => {',
    '          const ch = typeof feed === "string" ? feed : feed.channel_id;',
    '          const health = feed.health || "unknown";',
    '          const hoursLeft = feed.hours_until_expiry ? Math.floor(feed.hours_until_expiry) : 0;',
    '          const daysLeft = Math.floor(hoursLeft / 24);',
    '          let statusBadge = "";',
    '          if (health === "ok") {',
    '            statusBadge = `<span style="padding:2px 8px;border-radius:12px;font-size:11px;background:#d1fae5;color:#065f46;margin-left:8px;">✓ Active (${daysLeft}d ${hoursLeft % 24}h)</span>`;',
    '          } else if (health === "expiring_soon") {',
    '            statusBadge = `<span style="padding:2px 8px;border-radius:12px;font-size:11px;background:#fef3c7;color:#92400e;margin-left:8px;">⚠ Expiring soon</span>`;',
    '          } else if (health === "expired") {',
    '            statusBadge = `<span style="padding:2px 8px;border-radius:12px;font-size:11px;background:#fee2e2;color:#991b1b;margin-left:8px;">✗ Expired</span>`;',
    '          } else {',
    '            statusBadge = `<span style="padding:2px 8px;border-radius:12px;font-size:11px;background:#e5e7eb;color:#374151;margin-left:8px;">? Unknown</span>`;',
    '          }',
    '          feedsHtml +=',
    '            "<div class=\\"feed-item\\">" +',
    '              "<div>" +',
    ...
    '                statusBadge +',
    '                "<button class=\\"small\\" onclick=\\"renewSub(\'" + ch + "\')\\" style=\\"margin-left:8px;padding:4px 8px;font-size:11px;background:#3b82f6;color:white;border:none;border-radius:4px;cursor:pointer;\\">Renew</button>" +',
    '              "</div>" +',
    ...
    '    errSpan.textContent = "Lỗi tải accounts: " + e.message;',
    '    accountsEl.innerHTML = "";',
    '    accountsEl.appendChild(errSpan);',
    '  }',
    '}',
    ...
    '      const errSpan = document.createElement("span");',
    '      errSpan.style.color = "#b91c1c";',
    '      errSpan.textContent = "Lỗi gợi ý: " + err.message;',
    '      listEl.innerHTML = "";',
    '      listEl.appendChild(errSpan);',
    ...
    'window.renewSub = async function(channelId) {',
    '  if (!confirm("Renew subscription for " + channelId + "?")) return;',
    '  try {',
    '    setStatus("Renewing subscription...", true);',
    '    const result = await api("/admin/renew-subscription", {',
    '      method: "POST",',
    '      body: JSON.stringify({ channelId })',
    '    });',
    '    if (result.ok) {',
    '      setStatus("Subscription renewed successfully!", true);',
    '      await loadAccounts();  // Reload to show updated status',
    '    } else {',
    '      setStatus("Failed to renew: " + (result.error || "Unknown error"), false);',
    '    }',
    '  } catch (err) {',
    '    setStatus("Error renewing subscription: " + err.message, false);',
    '  }',
    '};',
    '',
    '// Test Video feature',
    ...
    '  if (!url) {',
    '    setStatus("Nhập URL video YouTube", false);',
    '    return;',
    '  }',
    '',
    '  btnTestVideo.disabled = true;',
    '  testResult.innerHTML = "Đang kiểm tra...";',
    '',
    '  try {',
    ...
    '      const errSpan = document.createElement("span");',
    '      errSpan.style.color = "#b91c1c";',
    '      errSpan.textContent = "Lỗi: " + checkRes.error;',
    '      testResult.innerHTML = "";',
    '      testResult.appendChild(errSpan);',
    '      return;',
    '    }',
    '',
    ...
    '    html += "<div><b>Video:</b> " + escapeHtml(videoInfo.title) + "</div>";',
    ...
    '    const errSpan = document.createElement("span");',
    '    errSpan.style.color = "#b91c1c";',
    '    errSpan.textContent = "Lỗi: " + err.message;',
    '    testResult.innerHTML = "";',
    '    testResult.appendChild(errSpan);',
    '    setStatus("Lỗi test video: " + err.message, false);',
    '  } finally {',
    ...
    '  if (!confirm("Gửi video này đến tất cả accounts Telegram?")) return;',
    '',
    '  try {',
    ...
    '      setStatus("✅ " + (res.message || "Đã gửi video test đến Telegram!"), true);',
    '    } else {',
    '      setStatus("❌ " + (res.error || "Không thể gửi video"), false);',
    '    }',
    '  } catch (err) {',
    '    setStatus("❌ Lỗi gửi video: " + err.message, false);',
    '  }',
    '}',
    '',
    'loadAccounts();',
